    ...
    , _core_dumped(false)
{
    ...
    _inpair[0]  = stdin_flag;
    _inpair[1]  = stdin_flag;
    _outpair[0] = stdout_flag;
    _outpair[1] = stdout_flag;
    _errpair[0] = stderr_flag;
    _errpair[1] = stderr_flag;
}
...
{
    std::string ret;
    for (std::size_t i = 0, l = _cxx_argv.size(); i < l; ++i) {
...
std::string read_all(int fd)
{
    ...
    while (true) {
        ...
        sbuf.sputn(buf, std::streamsize(strlen(buf)));
    }
    return sbuf.str();
}
...
int output(const MlmSubprocess::Argv& args, std::string& o, std::string& e, const std::string& i, uint64_t timeout,
    size_t timestep)
{
    SubProcess p(args, SubProcess::STDOUT_PIPE | SubProcess::STDERR_PIPE | SubProcess::STDIN_PIPE);
    p.run();
    int r = int(::write(p.getStdin(), i.c_str(), i.size()));
    ::fsync(p.getStdin());
    p.closeStdin();
    if (r == -1)
        return r;
...
std::string wait_read_all(int fd)
{
    ...
    memset(buf, '\0', BUF_SIZE + 1);
    errno = 0;
    while (::read(fd, buf, BUF_SIZE) <= 0 && (errno == EAGAIN || errno == EWOULDBLOCK) && exit < 5000) {
        usleep(1000);
        errno = 0;
        exit++;
    }
...
char* const* _mk_argv(const MlmSubprocess::Argv& vec)
{
    char** argv = reinterpret_cast<char**>(malloc(sizeof(char*) * (vec.size() + 1)));
    assert(argv);

    for (auto i = 0u; i != vec.size(); i++) {

        auto  str  = vec[i];
        char* dest = reinterpret_cast<char*>(malloc(sizeof(char) * (str.size() + 1)));
    ...
    return reinterpret_cast<char* const*>(argv);
}
...
static int xzloop_add_fd(zloop_t* self, int fd, zloop_fn handler, void* arg)
{
    assert(self);
    zmq_pollitem_t* fditem = reinterpret_cast<zmq_pollitem_t*>(zmalloc(sizeof(zmq_pollitem_t)));
    assert(fditem);
    fditem->fd     = fd;
...
static int s_handler(zloop_t* loop, zmq_pollitem_t* item, void* arg)
{
    ...
    ssize_t r = ::read(item->fd, buf, PIPE_BUF);
    i->buff << buf;
...
// stop the loop
static int s_end_loop(zloop_t* /*loop*/, int /*timer_id*/, void* /*args*/)
{
    return -1;
}
...
static int s_output(SubProcess& p, std::string& o, std::string& e, uint64_t timeout, size_t timestep)
{
    ...
    if (timeout != 0)
        zloop_timer(loop, timeout * 1000, 1, s_end_loop, nullptr);
...
int simple_output(const MlmSubprocess::Argv& args, std::string& o, std::string& e)
{
...
void fty_common_mlm_subprocess_test(bool verbose)
{
    ...
    // TEST_CASE ("subprocess-test-timeout3", "[subprocess][output]")
    {
        ...
        assert((delta >= 2000 &&
                delta < 5000)); // it's hard to tell how long the delay was, but it must be between 2 and 5 secs
    }
}
#endif
