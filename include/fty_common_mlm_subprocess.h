...
{
public:
    static constexpr int STDIN_PIPE            = 0x01;
    static constexpr int STDOUT_PIPE           = 0x02;
    static constexpr int STDERR_PIPE           = 0x04;
    static constexpr int STDERR_TO_STDOUT_PIPE = 0x08;

    static constexpr int PIPE_DEFAULT   = -1;
    static constexpr int PIPE_DISABLED  = -2;
    static constexpr int PIPE_TO_STDOUT = -3;
    ...
    {
        poll();
        return _state == SubProcessState::RUNNING;
    }
...
//
// \warning use internal reactor, so it regularly reads from stdout and stderr
int output(const Argv& args, std::string& o, std::string& e, const std::string& i, uint64_t timeout = 0,
    size_t timestep = 500);
...
//      negative is a number of a signal which terminates process
int simple_output(const Argv& args, std::string& o, std::string& e);
